...
apt -qq autoremove
apt-get -qq install -y apt-transport-https ca-certificates gnupg software-properties-common wget > /dev/null
wget -O - https://apt.kitware.com/keys/kitware-archive-latest.asc 2>/dev/null | apt-key add -
apt-add-repository 'deb https://apt.kitware.com/ubuntu/ bionic main' > /dev/null
apt-get -qq update
...
LD_LIBRARY_PATH="/usr/lib/python3.6/config-3.6m-x86_64-linux-gnu/:$LD_LIBRARY_PATH" /usr/bin/cmake --build buildpy --config Release --target surgepy > /dev/null
